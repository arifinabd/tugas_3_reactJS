import React, { Component } from "react";
import ListMakanan from "../ListData/ListMakanan";

class MenuMakanan extends Component{
    constructor(props){
        super(props);
        this.state ={
            pesan: 0
        }
        this.rubahPesan = this.rubahPesan.bind(this)
        this.pesan = this.pesan.bind(this)
    }
    rubahPesan(){
        this.setState((state, props) => {
            return {
                pesan: state.pesan
            }
        })
    }
    pesan(e){
        console.log(e.target.value)
    }

    render(){
        return(
            <div>
                <h3>Daftar Makan Yang Kami Sediakan :</h3>
                <table>
                    <tbody>
                        <tr>
                            <td>
                                <ListMakanan gambarLink="https://s3-ap-southeast-1.amazonaws.com/niomic/img/sample/nasipadang.jpg"/>
                                <center>
                                    <button type="button" onClick={this.rubahPesan}>Pesan Sekarang</button>
                                </center>
                            </td>
                            <td>
                                <ListMakanan gambarLink="https://s3-ap-southeast-1.amazonaws.com/niomic/img/sample/sate.png"/>
                                <center>
                                    <button type="button" onClick={this.rubahPesan}>Pesan Sekarang</button>
                                </center>
                            </td>
                            <td>
                                <ListMakanan gambarLink="https://s3-ap-southeast-1.amazonaws.com/niomic/img/sample/sotolamongan.png"/>
                                <center>
                                    <button type="button" onClick={this.rubahPesan}>Pesan Sekarang</button>
                                </center>
                            </td>
                            <td>
                                <ListMakanan gambarLink="https://www.dbs.com/iwov-resources/images/newsroom/indonesia/Blog/masakan%20nusantara/nasi%20kentut.png"/>
                                <center>
                                    <button type="button" onClick={this.rubahPesan}>Pesan Sekarang</button>
                                </center>
                            </td>
                            <td>
                                <ListMakanan gambarLink="https://blog.tokowahab.com/wp-content/uploads/2020/03/Resep-Nasi-Kuning-Tumpeng-Mini.jpg "/>
                                <center>
                                    <button type="button" onClick={this.rubahPesan}>Pesan Sekarang</button>
                                </center>
                            </td>
                        </tr>
                    </tbody>
                </table>
                <br/>
                <input type="text" onChange={this.pesan}/>
                <h3>Pesanan Anda : {this.state.pesan}</h3>
            </div>
        )
    }
}

export default MenuMakanan